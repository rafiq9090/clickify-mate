<template>
  <div class="min-h-screen bg-background flex items-center justify-center p-4 relative overflow-hidden font-sans">
    ...
    <!-- Auth Card -->
    <div class="w-full max-w-[400px] bg-surface/85 border border-outline p-6 md:p-10 rounded-[2.5rem] shadow-2xl z-10 relative animate-in fade-in zoom-in-95 duration-500 backdrop-blur-3xl">
        ...
        <form @submit.prevent="handleAuth" class="space-y-4">
            <div class="space-y-3">
                ...
                <div v-if="!isForgotPassword" class="relative group">
                    ...
                    />
                    <button type="button" @click="showPassword = !showPassword" class="absolute right-6 top-1/2 -translate-y-1/2 text-on-surface-variant/40 hover:text-primary transition-colors">
                        <span class="material-symbols-outlined text-xl">{{ showPassword ? 'visibility_off' : 'visibility' }}</span>
                    </button>
                </div>

                <div v-if="!isSignUp && !isForgotPassword" class="flex justify-end px-2">
                    <button type="button" @click="isForgotPassword = true" class="text-[10px] font-black uppercase tracking-widest text-on-surface-variant/60 hover:text-primary transition-colors">
                        Forgot Password?
                    </button>
                </div>
                    ...
                    <div v-if="otpCode.length >= 4" class="space-y-3 animate-in fade-in slide-in-from-top-2 duration-500">
                        ...
                        <div class="relative group">
                            ...
                            <input 
                                ...
                                class="w-full bg-surface pl-14 pr-14 py-4 rounded-2xl text-sm font-bold border border-outline focus:border-primary focus:ring-4 ring-primary/10 outline-none transition-all placeholder:text-on-surface-variant/40 text-on-surface"
                            />
                            <button type="button" @click="showConfirmPassword = !showConfirmPassword" class="absolute right-6 top-1/2 -translate-y-1/2 text-on-surface-variant/40 hover:text-primary transition-colors">
        ...
        <div class="mt-6 pt-6 border-t border-outline text-center space-y-4">
            <p class="text-[10px] font-black uppercase tracking-widest text-on-surface-variant/50">Or continue with</p>

            <div class="grid grid-cols-1 gap-4">
                <button @click="signInWithProvider('google')" class="flex items-center justify-center gap-3 py-3.5 bg-surface-hover/50 border border-outline rounded-2xl hover:bg-surface-hover shadow-sm transition-all group">
            ...
            <p class="text-[11px] font-bold text-on-surface-variant/70 pt-2 tracking-tight">
                ...
                <template v-else-if="allowSignup">
...
const allowSignup = computed(() => runtimeConfig.public.allowSignup !== false && runtimeConfig.public.allowSignup !== 'false')
...
const handleForgotPassword = async () => {
    ...
    try {
        ...
        const redirectUrl = isLocal 
            ...
            : 'https://clickifymate.com/auth/reset-password'
