...
export default defineEventHandler((event) => {
  ...
  return [
    ...
    '# HELP clickify_http_requests_total Total number of HTTP requests processed by dashboard.',
    '# TYPE clickify_http_requests_total counter',
    `clickify_http_requests_total ${requestCounter}`,
    ...
    `clickify_process_uptime_seconds ${Math.floor(process.uptime())}`,
    ''
  ].join('\n')
})
